...
def read_byte_with_retry(bus, addr, reg, max_retries=3, delay=0.01):
    ...
    for attempt in range(max_retries):
        try:
            ...
            time.sleep(delay)
    raise IOError(
        f"Failed to read byte from reg 0x{reg:02X} after {max_retries} attempts: {last_exception}"
    )
...
def write_byte_with_retry(bus, addr, reg, value, max_retries=3, delay=0.01):
    ...
    for attempt in range(max_retries):
        try:
            ...
            time.sleep(delay)
    raise IOError(
        f"Failed to write byte 0x{value:02X} to reg 0x{reg:02X} after {max_retries} attempts: {last_exception}"
    )
...
def read_word_with_retry(bus, addr, reg_lsb, max_retries=3, delay=0.01):
    ...
    for attempt in range(max_retries):
        try:
            ...
            time.sleep(delay)
    raise IOError(
        f"Failed to read word from LSB reg 0x{reg_lsb:02X} after {max_retries} attempts: {last_exception}"
    )
...
def write_word_with_retry(bus, addr, reg_lsb, value, max_retries=3, delay=0.01):
    ...
    for attempt in range(max_retries):
        try:
            ...
            time.sleep(delay)
    raise IOError(
        f"Failed to write word 0x{value:04X} to LSB reg 0x{reg_lsb:02X} after {max_retries} attempts: {last_exception}"
    )
...
def decode_status_1(status):
    ...
    bits["ADC_DONE_DESC"] = (
        "Complete (OneShot)" if bits["ADC_DONE_STAT"] else "Not Complete/Continuous"
    )
...
def decode_status_3(status):
    ...
    bits["REVERSE_STAT_DESC"] = (
        "Reverse Mode On" if bits["REVERSE_STAT"] else "Reverse Mode Off"
    )
...
def decode_fault_status(fault):
    ...
    active_faults = []
    if bits["VAC_UV_STAT"]:
        active_faults.append("Input UVP")
    if bits["VAC_OV_STAT"]:
        active_faults.append("Input OVP")
    if bits["IBAT_OCP_STAT"]:
        active_faults.append("Battery OCP")
    if bits["VBAT_OV_STAT"]:
        active_faults.append("Battery OVP")
    if bits["TSHUT_STAT"]:
        active_faults.append("Thermal Shutdown")
    if bits["CHG_TMR_STAT"]:
        active_faults.append("Charge Safety Timer Expired")
    if bits["DRV_OKZ_STAT"]:
        active_faults.append("DRV_SUP Fault")

    bits["ACTIVE_FAULTS_DESC"] = (
        ", ".join(active_faults) if active_faults else "No Active Faults"
    )
    return bits
    ...
    for name, (
        ...
        try:
            ...
            if not (min_v <= value <= max_v):
                warnings.append(
                    f"Warning: {name} value {value:.2f}{unit} outside expected range {min_v}-{max_v}{unit}"
                )
...
def configure_adc(bus, continuous=True, resolution_bits=15, average=False):
    ...
    if resolution_bits == 15:
        ...
        console.print(
            f"[yellow]Warning: Invalid resolution {resolution_bits} requested, using 15-bit.[/yellow]"
        )
        adc_ctrl_val |= ADC_CTRL_ADC_SAMPLE_15BIT
    ...
    try:
        ...
        console.print(
            f" - ADC Control (0x2B) written: 0x{adc_ctrl_val:02X}, Read back: 0x{verify_ctrl:02X}"
        )
    ...
    try:
        write_byte_with_retry(
            bus, BQ25756_ADDR, REG_ADC_CHANNEL_CTRL, adc_channel_ctrl_val
        )
        ...
        console.print(
            f" - ADC Channel Control (0x2C) written: 0x{adc_channel_ctrl_val:02X}, Read back: 0x{verify_channel:02X}"
        )
        if verify_channel != adc_channel_ctrl_val:
            console.print(
                "[red] - ADC Channel Control Write Verification FAILED![/red]"
            )
...
def set_charge_voltage_limit(bus, voltage_mv):
    ...
    if clamped_v != voltage_mv:
        console.print(
            f"[yellow]Charge Voltage Limit clamped from {voltage_mv}mV to {clamped_v}mV (Range: {min_v}-{max_v}mV)[/yellow]"
        )

    vfb_reg_val = (
        round((clamped_v - offset) / step) & 0x1F
    )  # Calculate value and mask to 5 bits

    reg_val_16bit = vfb_reg_val  # Bits 4:0 are the LSB bits of the 16-bit word

    console.print(
        f"Setting Charge Voltage Limit to {clamped_v}mV (VFB_REG = 0x{vfb_reg_val:02X})"
    )
    try:
        ...
        write_word_with_retry(
            bus, BQ25756_ADDR, REG_CHARGE_VOLTAGE_LIMIT, reg_val_16bit
        )
...
def set_charge_current_limit(bus, current_ma):
    ...
    if clamped_c != current_ma:
        console.print(
            f"[yellow]Charge Current Limit clamped from {current_ma}mA to {clamped_c}mA (Range: {min_c}-{max_c}mA)[/yellow]"
        )

    ichg_reg_val = (
        round(clamped_c / step) & 0x1FF
    )  # Calculate value and mask to 9 bits (10:2 -> 9 bits)

    # Shift value into correct bit positions (10:2)
    reg_val_16bit = ichg_reg_val << 2

    console.print(
        f"Setting Charge Current Limit to {clamped_c}mA (ICHG_REG = 0x{ichg_reg_val:03X}) -> Writing 0x{reg_val_16bit:04X}"
    )
    try:
        write_word_with_retry(
            bus, BQ25756_ADDR, REG_CHARGE_CURRENT_LIMIT, reg_val_16bit
        )
...
def set_input_current_dpm_limit(bus, current_ma):
    ...
    if clamped_c != current_ma:
        console.print(
            f"[yellow]Input Current Limit clamped from {current_ma}mA to {clamped_c}mA (Range: {min_c}-{max_c}mA)[/yellow]"
        )
    ...
    console.print(
        f"Setting Input Current Limit to {clamped_c}mA (IAC_DPM = 0x{iac_dpm_val:03X}) -> Writing 0x{reg_val_16bit:04X}"
    )
    try:
        write_word_with_retry(
            bus, BQ25756_ADDR, REG_INPUT_CURRENT_DPM_LIMIT, reg_val_16bit
        )
...
def set_input_voltage_dpm_limit(bus, voltage_mv):
    ...
    if clamped_v != voltage_mv:
        console.print(
            f"[yellow]Input Voltage Limit clamped from {voltage_mv}mV to {clamped_v}mV (Range: {min_v}-{max_v}mV)[/yellow]"
        )

    # Calculate the raw value based on 0mV origin
    vac_dpm_val = (
        round(clamped_v / step) & 0xFFF
    )  # Calculate value and mask to 12 bits (13:2 -> 12 bits)

    # Shift value into correct bit positions (13:2)
    reg_val_16bit = vac_dpm_val << 2

    console.print(
        f"Setting Input Voltage DPM Limit to {clamped_v}mV (VAC_DPM = 0x{vac_dpm_val:03X}) -> Writing 0x{reg_val_16bit:04X}"
    )
    try:
        write_word_with_retry(
            bus, BQ25756_ADDR, REG_INPUT_VOLTAGE_DPM_LIMIT, reg_val_16bit
        )
...
def enable_charging(bus, enable=True):
    ...
    try:
        ...
        if new_val == current_val:
            console.print(
                f" - Charging already {action.lower().replace('ing', 'ed')}. Value: 0x{current_val:02X}"
            )
            return
        ...
        console.print(
            f" - Charger Control (0x17) written: 0x{new_val:02X}, Read back: 0x{verify_val:02X}"
        )
...
def set_battery_only_mode(bus):
    ...
    try:
        ...
        if current_charger_ctrl != new_charger_ctrl:
            ...
            console.print(
                f" - Charger Control (0x17): Wrote 0x{new_charger_ctrl:02X}, Read 0x{verify_charger:02X}"
            )
            if verify_charger != new_charger_ctrl:
                success = False
                console.print("[red]   Verification Failed![/red]")
        else:
            console.print(
                f" - Charger Control (0x17): Already 0x{current_charger_ctrl:02X} (Charge Disabled)"
            )
    except Exception as e:
        success = False
    ...
    try:
        current_power_path_ctrl = read_byte_with_retry(
            bus, BQ25756_ADDR, REG_POWER_PATH_REVERSE_MODE_CTRL
        )
        new_power_path_ctrl = current_power_path_ctrl & ~(
            (1 << 0) | (1 << 6)
        )  # Clear EN_REV (bit 0), EN_IAC_LOAD (bit 6)
        if current_power_path_ctrl != new_power_path_ctrl:
            write_byte_with_retry(
                bus, BQ25756_ADDR, REG_POWER_PATH_REVERSE_MODE_CTRL, new_power_path_ctrl
            )
            verify_power = read_byte_with_retry(
                bus, BQ25756_ADDR, REG_POWER_PATH_REVERSE_MODE_CTRL
            )
            console.print(
                f" - Power Path Control (0x19): Wrote 0x{new_power_path_ctrl:02X}, Read 0x{verify_power:02X}"
            )
            if verify_power != new_power_path_ctrl:
                success = False
                console.print("[red]   Verification Failed![/red]")
        else:
            console.print(
                f" - Power Path Control (0x19): Already 0x{current_power_path_ctrl:02X} (Reverse/VACLoad Disabled)"
            )
    except Exception as e:
        success = False
    ...
    if success:
        ...
        console.print(
            "[red]Failed to fully apply battery-only mode configuration.[/red]"
        )
...
def generate_status_table(bus):
    ...
    try:
        ...
        table.add_row(
            "Charger Status 1",
            f"{status1_decoded['CHARGE_STAT_DESC']}",
            f"0x{status1_raw:02X} @ 0x21",
        )
        if status1_decoded["VAC_DPM_STAT"]:
            table.add_row("Input Voltage DPM", "[yellow]Active[/yellow]", "")
        if status1_decoded["IAC_DPM_STAT"]:
            table.add_row("Input Current DPM", "[yellow]Active[/yellow]", "")
        if status1_decoded["WD_STAT"]:
            table.add_row("Watchdog Status", "[red]Expired[/red]", "")
    except Exception as e:
    ...
    try:
        ...
        table.add_row(
            "Power Good (PG)",
            f"[{pg_style}]{status2_decoded['PG_STAT_DESC']}[/]",
            f"0x{status2_raw:02X} @ 0x22",
        )
    ...
    try:
        ...
        table.add_row(
            "Reverse Mode",
            f"{status3_decoded['REVERSE_STAT_DESC']}",
            f"0x{status3_raw:02X} @ 0x23",
        )
        if status3_decoded["CV_TMR_STAT"]:
            table.add_row("CV Timer", "[yellow]Expired[/yellow]", "")
    ...
    try:
        ...
        fault_style = (
            "red"
            if fault_decoded["ACTIVE_FAULTS_DESC"] != "No Active Faults"
            else "green"
        )
        table.add_row(
            "Fault Status",
            f"[{fault_style}]{fault_decoded['ACTIVE_FAULTS_DESC']}[/]",
            f"0x{fault_raw:02X} @ 0x24",
        )
    except Exception as e:
    ...
    return Panel(
        table,
        title=f"Status @ {datetime.now().strftime('%H:%M:%S.%f')[:-3]}",
        border_style="blue",
    )
...
def monitor_status_live(bus, duration_sec=30, refresh_rate_hz=1):
    ...
    try:
        with Live(
            generate_status_table(bus), refresh_per_second=refresh_rate_hz, screen=False
        ) as live:
...
def set_reverse_mode_voltage_limit(bus, voltage_mv):
    ...
    if clamped_v != voltage_mv:
        console.print(
            f"[yellow]Reverse Mode Voltage Limit clamped from {voltage_mv}mV to {clamped_v}mV (Range: {min_v}-{max_v}mV)[/yellow]"
        )
    ...
    console.print(
        f"Setting Reverse Mode Voltage Limit to {clamped_v}mV (VAC_REV = 0x{vac_rev_val:03X}) -> Writing 0x{reg_val_16bit:04X}"
    )
    try:
        write_word_with_retry(
            bus, BQ25756_ADDR, REG_REVERSE_MODE_INPUT_VOLTAGE_LIMIT, reg_val_16bit
        )
        read_val = read_word_with_retry(
            bus, BQ25756_ADDR, REG_REVERSE_MODE_INPUT_VOLTAGE_LIMIT
        )
        console.print(
            f" - Reverse Mode Voltage Regs (0x0C/0D) Read back: 0x{read_val:04X}"
        )
        if read_val != reg_val_16bit:
            console.print(
                "[red] - Reverse Mode Voltage Write Verification FAILED![/red]"
            )
            return False
        return True
    except Exception as e:
...
def set_reverse_mode_current_limit(bus, current_ma):
    ...
    if clamped_c != current_ma:
        console.print(
            f"[yellow]Reverse Mode Current Limit clamped from {current_ma}mA to {clamped_c}mA (Range: {min_c}-{max_c}mA)[/yellow]"
        )
    ...
    console.print(
        f"Setting Reverse Mode Current Limit to {clamped_c}mA (IAC_REV = 0x{iac_rev_val:03X}) -> Writing 0x{reg_val_16bit:04X}"
    )
    try:
        write_word_with_retry(
            bus, BQ25756_ADDR, REG_REVERSE_MODE_INPUT_CURRENT_LIMIT, reg_val_16bit
        )
        read_val = read_word_with_retry(
            bus, BQ25756_ADDR, REG_REVERSE_MODE_INPUT_CURRENT_LIMIT
        )
        console.print(
            f" - Reverse Mode Current Regs (0x0A/0B) Read back: 0x{read_val:04X}"
        )
        if read_val != reg_val_16bit:
            console.print(
                "[red] - Reverse Mode Current Write Verification FAILED![/red]"
            )
            return False
        return True
    except Exception as e:
...
def set_reverse_mode(bus, enable, target_voltage_mv=5000, target_current_ma=1000):
    ...
    if enable:
        ...
        if not (v_ok and c_ok):
            console.print(
                "[red]Failed to set Reverse Mode limits. Aborting enable.[/red]"
            )
            return False
        ...
        try:
            ...
            if new_val == current_val:
                ...
                verify_val = read_byte_with_retry(bus, BQ25756_ADDR, reg_addr)
                console.print(
                    f" - Power Path Control (0x19) written: 0x{new_val:02X}, Read back: 0x{verify_val:02X}"
                )
                if verify_val != new_val:
                    console.print(
                        "[red] - Enable Reverse Mode Write Verification FAILED![/red]"
                    )
                    return False
        except Exception as e:
        ...
        try:
            ...
            if new_val == current_val:
                ...
                if verify_val != new_val:
                    console.print(
                        "[red] - Disable Reverse Mode Write Verification FAILED![/red]"
                    )
                    return False
        except Exception as e:
    ...
    try:
        ...
        if chrgr_flag2_val & 0x08:
            console.print("Reverse mode toggle detected")
        else:
        ...
        if reverse_stat == expected_stat:
            console.print(
                f"[green]Reverse Mode {action} successful. REVERSE_STAT (Reg 0x23, Bit 2) is now {reverse_stat}.[/green]"
            )
            return True
        else:
            console.print(
                f"[red]Reverse Mode {action} verification failed! REVERSE_STAT (Reg 0x23, Bit 2) is {reverse_stat} (expected {expected_stat}).[/red]"
            )
            return False
    except Exception as e:
...
def configure_i2c_mux(bus, mux_address=DEFAULT_MUX_ADDRESS, channel=None):
    ...
    if channel is None:
        ...
        console.print(
            f"[red]Invalid channel {channel} requested for TCA9548A (must be 0-7 or None).[/red]"
        )
        return False

    console.print(
        f"[bold cyan]{action_desc} I2C Mux @ 0x{mux_address:02X} (Control Byte: 0x{control_byte:02X})...[/bold cyan]"
    )
    ...
    try:
        ...
        if read_back == control_byte:
            console.print(
                f"[green] - Mux configuration successful (Read back: 0x{read_back:02X}).[/green]"
            )
            return True
        else:
            console.print(
                f"[red] - Mux config write sent (0x{control_byte:02X}), but read back failed (0x{read_back:02X}). Check mux address and connections.[/red]"
            )
            return False  # Treat mismatch as failure for TCA9548A

    except Exception as e:
        console.print(
            f"[red]Error configuring I2C Mux @ 0x{mux_address:02X}: {e}[/red]"
        )
        return False
...
if __name__ == "__main__":
    console.print("[bold]BQ25756 Bring-up Script (Datasheet: SLUSEN5)[/bold]")
    console.print(
        f"Attempting to use I2C bus {I2C_BUS_NO}, Address 0x{BQ25756_ADDR:02X}"
    )
    ...
    try:
        ...
        # --- Configure I2C Mux ---
        configure_i2c_mux(
            bus, mux_address=DEFAULT_MUX_ADDRESS, channel=BUCK_BOOST_CHANNEL
        )
        ...
        try:
            ...
            console.print(
                f"Read Part Info (0x3D): 0x{part_info:02X} -> Part#: {part_num}, Rev: {dev_rev}"
            )
            # According to Table 8-49, Part# 010 (decimal 2) is BQ25756
            if part_num != 0b010:
                console.print(
                    f"[bold red]WARNING: Detected Part Number ({part_num}) does not match BQ25756 (expected 2)! Check I2C Address and Device.[/bold red]"
                )
                # sys.exit(1) # Optional: Exit if wrong device detected
        except Exception as e:
            console.print(
                f"[bold red]FATAL: Failed to read Part Info from 0x{BQ25756_ADDR:02X}. Check I2C bus, address, and device power.[/bold red]"
            )
        ...
        set_charge_voltage_limit(
            bus, 1536
        )  # Example: Set to reset default 1536mV (VFB) -> Corresponds to actual Batt voltage via divider
        ...
        set_input_voltage_dpm_limit(
            bus, 4200
        )  # Example: 4200mV input voltage DPM (VINDPM threshold)
        ...
        try:
            ...
            if pg_stat == 1:
                console.print(
                    "[bold yellow]Warning: Adapter detected (PG_STAT = 1). Chip may prevent entering Reverse Mode.[/bold yellow]"
                )
            else:
                console.print(
                    "[bold green]Adapter not detected (PG_STAT = 0). OK to attempt Reverse Mode.[/bold green]"
                )
        except Exception as e:
        ...
        if set_reverse_mode(
            bus,
            enable=True,
            target_voltage_mv=TARGET_REV_V,
            target_current_ma=TARGET_REV_I,
        ):
        ...
        console.print(
            f" - If I2C bus number '{I2C_BUS_NO}' is correct for your system."
        )
        console.print(
            f" - If the BQ25756 device is connected and powered at address 0x{BQ25756_ADDR:02X}."
        )
        console.print(
            " - If the user running this script has I2C permissions (e.g., part of 'i2c' group)."
        )
    except Exception as e:
