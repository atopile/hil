...
class Cell:
    ...
    async def set_mux(self, verbose=False):
        ...
        if verbose:
            print(
                f"[Cell {self.cell_num}] MUX set: channel {self.mux_channel} (value: {hex(value)})"
            )
    ...
    async def init(self):
        ...
        self.adc = await ADS1115.create(1)
    ...
    async def calculate_setpoint(self, voltage, use_buck_calibration=True):
        ...
        """
        calibration = (
            self.BUCK_SETPOINTS if use_buck_calibration else self.LDO_SETPOINTS
        )
